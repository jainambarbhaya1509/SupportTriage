...
def test_reply_without_message_uses_groq_when_configured(monkeypatch) -> None:
    env = SupportTriageEnvironment()
    env.reset(task_id="billing_refund_easy")
    ...
    monkeypatch.setattr(
        env_module,
        "generate_groq_reply",
        lambda api_key, model, observation, ticket_id: "I have escalated this billing refund request for review.",
    )
...
def test_auto_baseline_falls_back_to_scripted_without_groq_key(monkeypatch) -> None:
